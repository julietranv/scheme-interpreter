...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        eval_op = operands.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(self, eval_op, env)
        # END PROBLEM 5
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        opers = Thunk(complete_eval(scheme_apply(self, operands, env)), env)
        return complete_eval(opers)
        # END Problem 21
...
def do_and_form(expressions, env):
    ...
    if not value and not value is 0:
        return False
    elif expressions.second is nil:
        return value
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            if expressions.second is nil and expressions.first == 'else':
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        pair = bindings.first
        check_form(pair, 2, 2)
        symbols = Pair(pair.first, symbols)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            operator = complete_eval(scheme_eval(first, result.env))
            check_procedure(operator)
            result = operator.eval_call(rest, result.env)
            # END PROBLEM 20
    return result
